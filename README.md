...
Скрипты и сырые результаты бенчмарка для статьи
**«Ваши тесты медленные не из-за базы данных. Я измерил»** (ссылка будет после публикации).

Проект: бэкенд на Litestar + SQLAlchemy (asyncpg) + Postgres + Redis, 3316 тестов,
почти все интеграционные — ходят в настоящую БД через реальный ASGI-сервер.
Все замеры локальные, на одном ноутбуке, медиана из нескольких прогонов.
